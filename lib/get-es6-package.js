...
function getES6Package(packages, packageName, opts) {
  ...
  */
  ...
  if (pkg.isTypeScript) {
    libTree = typeScriptCompiler(libTree, packagePath);
  }

  var testFiles = [ jsRegExp ];
  ...
  if (pkg.isTypeScript) {
    testTree = typeScriptCompiler(testTree, packagePath);
  }
