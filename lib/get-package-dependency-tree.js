...
function packageDependencyTree(packages, packageName, opts) {
  ...
  vendoredDependencies.forEach(function(dependency) {
    var result = vendoredPackages[dependency];
    if (typeof result === 'object' && result !== null) {
      vendorTrees.push(result);
    } else {
      throw new TypeError('vendoredDependency `' + dependency +
                          '` is missing. It is dependent on by `' + packageName + '`');
    }
  });
